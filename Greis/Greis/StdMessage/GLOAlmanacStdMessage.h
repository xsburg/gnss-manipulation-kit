...
namespace Greis
{
    ...
    public:
        ...
        GLOAlmanacStdMessage(const std::string& p_id, int p_size);
        GLOAlmanacStdMessage(int p_size);
        
        struct Codes
        {
            static const std::string Code_NA;
        };
        
        virtual std::string ToString() const;
        virtual std::string Id() const { return _id; }
        virtual EMessageId::Type IdNumber() const { return EMessageId::GLOAlmanac; }
        ...
        // Satellite orbit slot number within [1…32] []
